...
class ParserActBuilderData:
    _hx_class_name = "ParserActBuilderData"
    __slots__ = ()
    _hx_statics = ["main", "parserRole"]

    @staticmethod
    def main():
        dir = "indexFile/roledata"
        ...
        while (_g_current < len(_g_array)):
            ...
            index = _g1_key
            value = _g1_value
            ParserActBuilderData.parserRole(((("null" if dir is None else dir) + "/") + ("null" if value is None else value)))

    @staticmethod
    def parserRole(path):
        haxe_Log.trace("开始解析：",_hx_AnonObject({'fileName': "ParserActBuilderData.hx", 'lineNumber': 17, 'className': "ParserActBuilderData", 'methodName': "parserRole", 'customParams': [path]}))
        startIndex1 = None
        pos = None
        if (startIndex1 is None):
            pos = path.rfind("/", 0, len(path))
        else:
        ...
        name = HxString.substr(path,(pos + 1),None)
        startIndex1 = None
        _hx_len = None
        if (startIndex1 is None):
            _hx_len = name.rfind(".", 0, len(name))
        else:
            i = name.rfind(".", 0, (startIndex1 + 1))
            startLeft = (max(0,((startIndex1 + 1) - len("."))) if ((i == -1)) else (i + 1))
            check = name.find(".", startLeft, len(name))
            _hx_len = (check if (((check > i) and ((check <= startIndex1)))) else i)
        name = HxString.substr(name,0,_hx_len)
        Sys.command((((("neko bin/actbuilder_parser.n " + ("null" if path is None else path)) + " Out/") + ("null" if name is None else name)) + "_AB"))
...
class Sys:
    _hx_class_name = "Sys"
    __slots__ = ()
    _hx_statics = ["systemName", "command"]
